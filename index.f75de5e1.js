...
refs.appendRow.addEventListener("click", ()=>{
    if (refs.field.rows.length === 10) return;
...
refs.appendColumn.addEventListener("click", ()=>{
    if (refs.field.rows[0]?.cells.length === 10) return;
...
refs.removeColumn.addEventListener("click", ()=>{
    for (const row of refs.field.rows)row.deleteCell(row.cells[row.cells?.length - 1]);
    isDisabledButton();
});
isDisabledButton();
